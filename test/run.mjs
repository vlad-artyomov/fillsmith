...
const strength = await page.evaluate(() => {
  ...
  const tag = (l) => { const d = G.matchRuleDetail(l, p); return d ? (d.weak ? 'weak' : 'strong') : 'none'; };
  return {
...
const unlocked = await page.evaluate(() => {
  ...
  const ask = (l) => { const v = G.matchRule(l, p); return Array.isArray(v) ? v[0] : v; };
  return { colour: ask('Farbe'), product: ask('Gerätebezeichnung'), role: ask('Funktion'),
...
const alone = await (async () => {
  ...
  const out = await solo.evaluate(() => {
    ...
    return { name: p.fullName, job: p.jobTitle, color: p.color, product: p.productName,
