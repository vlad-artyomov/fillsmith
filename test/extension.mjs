...
const watchdog = setTimeout(() => { console.log('\nWATCHDOG: suite exceeded 100s, aborting'); process.exit(1); }, 100000);
watchdog.unref?.();
...
if (worker) {
  ...
  const status = await withTimeout(worker.evaluate(async () => {
    ...
    try { return await LM.availability(); } catch (e) { return 'error: ' + e.message; }
  }), 10000, 'availability');
  ...
  const lateDbg = await pop.evaluate(async () => {
    ...
    fill.modelDebug = { at: Date.now() - 60000, asked: 1, waitedMs: 1500,
  ...
  const live = await pop.evaluate(async () => {
    ...
    const box = document.getElementById('result');
    return { hidden: box.hidden, rows: box.querySelectorAll('.act').length,
  ...
  const doubled = await withTimeout(worker.evaluate(async (files) => {
    ...
    await chrome.scripting.executeScript({
      target: { tabId: tab.id }, func: () => { globalThis.__formforgeRuns = 0; }
    });
  ...
  const verdicts = await withTimeout(worker.evaluate(async () => {
    const run = async (reply, slow) => {
      const fake = () => ({
        ...
        async prompt() { await new Promise(r => setTimeout(r, slow ? 3200 : 60)); return reply; },
        async clone() { return fake(); },
        destroy() {}
      });
      const real = self.LanguageModel;
      self.LanguageModel = { async availability() { return 'available'; }, async create() { return fake(); } };
      nanoSession = null;
  ...
  const sessions = await withTimeout(worker.evaluate(async () => {
    ...
    const fake = () => ({
      ...
      async prompt() { return '{"values":[{"id":1,"value":"ok"}]}'; },
      async clone() { clones++; return fake(); },
      destroy() {}
    });
    const realLM = self.LanguageModel;
    self.LanguageModel = {
      async availability() { return 'available'; },
      async create() { creates++; await new Promise(r => setTimeout(r, 300)); return fake(); }
    };
    nanoSession = null;
  ...
  const booted = await withTimeout(worker.evaluate(async ({ files, url }) => {
    ...
    const read = async () => (await chrome.scripting.executeScript({
      target: { tabId: tab.id }, func: () => {
        ...
        return { boxes: document.querySelectorAll('#formforge-hud').length,
    ...
    await chrome.scripting.executeScript({ target: { tabId: tab.id, allFrames: true }, files });
    await chrome.tabs.sendMessage(tab.id, { kind: 'fill',
  ...
  const warming = await withTimeout(worker.evaluate(async () => {
    ...
    self.LanguageModel = {
      ...
      create: async () => { await slow; return { prompt: async () => '{"values":[]}', destroy() {} }; }
    };
    const t0 = Date.now();
    const res = await generate({ persona: { fullName: 'W' }, pageTitle: 'w',
  ...
  const firstFill = await withTimeout(worker.evaluate(async ({ files, url, cold }) => {
    ...
    self.LanguageModel = {
      availability: async () => 'available',
      create: async () => {
        if (!built) { await new Promise(r => setTimeout(r, cold)); built = true; }
        return {
    ...
    nanoSession = null; nanoPending = null; nanoBuilding = false;
    const tab = await chrome.tabs.create({ url, active: false });
    await new Promise(r => setTimeout(r, 600));
    await chrome.scripting.executeScript({ target: { tabId: tab.id, allFrames: true }, files });
    const res = await chrome.tabs.sendMessage(tab.id, { kind: 'fill',
      settings: { locale: 'de-DE', useAI: true, overwrite: true } });
    await chrome.tabs.remove(tab.id);
    self.LanguageModel = real; nanoSession = realSession;
    return { aiUsed: res.aiUsed, warming: !!res.modelWarming, via: res.modelVia,
  ...
  const patience = await withTimeout(worker.evaluate(async ({ files, url }) => {
    ...
    self.LanguageModel = {
      availability: async () => 'available',
      create: async () => {
        if (!built) { await new Promise(r => setTimeout(r, 6000)); built = true; }
        return {
          clone: async function () { return { ...this }; },
          prompt: async (p) => {
            const first = !asked; asked = true;
    ...
    await new Promise(r => setTimeout(r, 600));
    await chrome.scripting.executeScript({ target: { tabId: tab.id, allFrames: true }, files });
    const res = await chrome.tabs.sendMessage(tab.id, { kind: 'fill', settings: {
      locale: 'de-DE', useAI: true, overwrite: true, modelTimeout: 15 } });
    await chrome.tabs.remove(tab.id);
    self.LanguageModel = real; nanoSession = realSession;
    return { aiUsed: res.aiUsed, timedOut: !!res.modelTimedOut, phase: res.phase,
             fallbacks: (res.filled || []).filter(f => String(f.source).startsWith('fallback')).length };
  ...
  const spin = await withTimeout(worker.evaluate(async () => {
    ...
    const ink = frames.map(f => {
      ...
      for (let y = 6; y < 26; y++) {
        for (let x = 2; x < 6; x++) {
          ...
          if (f.data[i + 3] > 200) { sum += f.data[i + 1]; n++; }
        }
      }
    ...
    return { frames: frames.length, distinct, ran, moved, stopped: !spinTimer, opaque,
  ...
  const batched = await withTimeout(worker.evaluate(async () => {
    ...
    const ms = Date.now() - t0;
    self.LanguageModel = real; nanoSession = realSession;
    return { ms, perCall: PER_CALL, batches: r.debug.batches.length,
  ...
  const restore = await withTimeout(worker.evaluate(async () => {
    ...
    const tell = (details) => new Promise(r => {
      try { chrome.action.setIcon(details, () => r(chrome.runtime.lastError
    ...
    stopSpin(tab.id);
    return {
      real: await tell({ tabId: tab.id, path: { 16: '/icons/icon16.png',
  ...
  const quiet = await withTimeout((async () => {
    const noisy = [];
    const watch = (m) => { if (m.type() === 'warning' || m.type() === 'error') noisy.push(m.type() + ': ' + m.text()); };
    ...
    const res = await worker.evaluate(async ({ files, url }) => {
      ...
      return await chrome.tabs.sendMessage(t.id, { kind: 'fill', settings: {
