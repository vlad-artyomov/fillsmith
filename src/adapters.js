...
(function () {
  ...
  const LIBS = [
    ...
    { id: 'primevue-datepicker', kind: 'date', root: '.p-datepicker:not(.p-datepicker-panel):not(.p-datepicker-inline)', input: '.p-datepicker-input, input' },
    ...
    { id: 'primevue-toggleswitch', kind: 'bool', root: '.p-toggleswitch', input: 'input.p-toggleswitch-input, input' },
    ...
    { id: 'primevue-selectbutton', kind: 'inline-choice', root: '.p-selectbutton', option: '.p-togglebutton, [role="option"], [role="radio"]' },
    { id: 'primevue-listbox', kind: 'inline-choice', root: '.p-listbox', option: '.p-listbox-option, li[role="option"]' },
    { id: 'primevue-rating', kind: 'inline-choice', root: '.p-rating', option: '.p-rating-option' },

    { id: 'mui-select', kind: 'choice', root: '.MuiSelect-root, .MuiAutocomplete-root', overlay: '.MuiPopover-paper, .MuiAutocomplete-popper', option: 'li[role="option"], .MuiMenuItem-root' },
    { id: 'antd-select', kind: 'choice', root: '.ant-select', label: '.ant-select-selection-item', overlay: '.ant-select-dropdown', option: '.ant-select-item-option' },
    { id: 'antd-picker', kind: 'date', root: '.ant-picker', input: 'input' },
    { id: 'react-select', kind: 'choice', root: '[class*="-container"]:has(> [class*="-control"]), .select__control', overlay: '[class*="-menu"], .select__menu', option: '[class*="-option"], .select__option' },
    { id: 'radix-select', kind: 'choice', root: '[data-radix-select-trigger], button[role="combobox"][aria-haspopup="listbox"]', overlay: '[data-radix-select-content], [data-radix-popper-content-wrapper]', option: '[role="option"]' },
    { id: 'headless-listbox', kind: 'choice', root: '[id^="headlessui-listbox-button"]', overlay: '[id^="headlessui-listbox-options"]', option: '[role="option"]' },
    { id: 'choices-js', kind: 'choice', root: '.choices', overlay: '.choices__list--dropdown', option: '.choices__item--choice' },
    { id: 'select2', kind: 'choice', root: '.select2-container', overlay: '.select2-dropdown', option: '.select2-results__option' },
    { id: 'tom-select', kind: 'choice', root: '.ts-wrapper', overlay: '.ts-dropdown', option: '.option' },
    { id: 'vue-multiselect', kind: 'choice', root: '.multiselect', overlay: '.multiselect__content-wrapper', option: '.multiselect__option' },

    // Last resort: anything that declares itself a combobox, switch or radio group.
    { id: 'aria-combobox', kind: 'choice', root: '[role="combobox"][aria-haspopup="listbox"], [role="combobox"][aria-expanded]', option: '[role="option"]' },
  ...
  function groupRadios(widgets) {
    ...
    for (const w of widgets) {
      if (w.kind !== 'radio') { out.push(w); continue; }
      const input = w.root.querySelector('input[type="radio"], input');
      const name = input && input.name;
      if (!name) { out.push(w); continue; }
  ...
  function radioLabel(root, input) {
    if (input) {
      if (input.labels && input.labels.length) { const t = textOf(input.labels[0]); if (t) return t; }
      if (input.id) {
  ...
  function labelFor(widget, describe) {
    ...
    if (!bits.some(Boolean)) {
      ...
      for (let i = 0; up && i < 3; i++, up = up.parentElement) {
        ...
        if (l && !l.contains(root)) { const t = textOf(l); if (t) { bits.push(t); break; } }
      }
    }
