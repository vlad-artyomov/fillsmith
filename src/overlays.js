...
(function () {
  ...
  const { note, sleep, visible, textOf, norm, press, key, settle, waitFor, safeQuery, PLACEHOLDER } = globalThis.FormForgeDom;
  ...
  function linkedOverlays(widget) {
    ...
    for (const attr of ['aria-controls', 'aria-owns']) {
      ...
      if (id) id.split(/\s+/).forEach(x => { const n = document.getElementById(x); if (n) out.push(n); });
    }
    return out;
  }
  ...
  async function openOverlay(widget) {
    ...
    const verdict = () => {
      ...
      if (hit) return hit;
      const empty = any();
      if (!empty) { emptySince = 0; return null; }
    ...
    if (overlay) try { overlay.setAttribute('data-formforge-opened', ''); } catch (_) { }
    return overlay;
  }
  ...
  async function closeOverlay(widget) {
    ...
    const stillOpen = () => {
      ...
      return ownOverlays(widget).some(o => optionsIn(o, widget.lib).length);
    };
    for (let i = 0; i < 3; i++) {
  ...
  async function huntByScrolling(overlay, lib, candidates, budgetEnds) {
    ...
    for (let i = 0; i < 80 && Date.now() < deadline; i++) {
      ...
      if (barren >= 2) { exhausted = true; break; }
      ...
      if (scroller.scrollTop <= was) {
        ...
        if (++stalled >= 2) { exhausted = true; break; }
        await sleep(140);
      } else stalled = 0;
    }
  ...
  async function dismissPanel(widget, opts) {
    ...
    const mayEscape = !(opts && opts.keepTypedValue);
    for (let i = 0; i < 4; i++) {
