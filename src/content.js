...
(function () {
  ...
  // Choices worth a longer search for a match: a country picker beside a phone number.
  const MUST_MATCH = /countrylist|countrycode|phonecountry|\b(country|land|staat)\b/i;
  ...
  function describe(el) {
    ...
    if (bits.filter(Boolean).length < 2) {
      ...
      for (let hops = 0; prev && hops < 3; hops++, prev = prev.previousElementSibling) {
        const t = textOf(prev);
        if (t && t.length < 80) { bits.push(t); break; }
      }
    }
  ...
  function limitsOf(el) {
    if (!el) return { placeholder: '', maxLength: null, pattern: null, min: null, max: null, step: null, autocomplete: '' };
  ...
  function popupSurfaces() {
    const roots = [];
    const add = (el) => { if (el && el !== document.body && !roots.includes(el)) roots.push(el); };
  ...
  function collectFields(opts) {
    ...
    const inPopup = (el) => popups.some(p => p.contains(el));
    ...
    inDomOrder(fields);
    fields.forEach((f, i) => { f.idx = i; try { f.el.setAttribute(MARK, String(i)); } catch (_) {} });
    return fields;
  }
  ...
  // ---------------------------------------------------------------- write ----
  const fire = (el, types) => { for (const t of types) el.dispatchEvent(new Event(t, { bubbles: true, composed: true })); };
  ...
  async function applyValue(f, rawValue, persona) {
    if (f.kind === 'widget') {
      return await W.fill(f.widget, G.constrain(shapeFor(f, rawValue, persona), f), {
        ...
        required: !!f.required,
        requireMatch: MUST_MATCH.test(f.label || ''),
    ...
    try {
      ...
      if (f.type === 'radio') {
        ...
        H.press(target);
        if (!target.checked) { target.checked = true; fire(target, ['input', 'change']); }
  ...
  function flash(el, ok) {
    ...
    el.style.outline = ok ? '2px solid #2f9e6f' : '2px solid #d08a2b';
  ...
  const sendMessage = (msg) => new Promise(r => {
    try { chrome.runtime.sendMessage(msg, (v) => { void chrome.runtime.lastError; r(v || {}); }); }
    catch (_) { r({}); }
  });
  ...
  async function askModel(unresolved, persona) {
    ...
    try {
      ...
      const res = await Promise.race([
        ...
        new Promise(r => setTimeout(() => r({ ok: false, timedOut: true }), budget + (loading ? SESSION_ALLOWANCE_MS : 0)))
      ]);
      modelLoading = false;
  ...
  async function run(settings) {
    ...
    if (modelSettings.useAI !== false) {
      sendMessage({ kind: 'nano-warm' }).then(r => { if (r && r.ready) modelWarm = true; });
    }
    ...
    document.querySelectorAll(O.PANEL_SELECTOR).forEach(el => { if (H.visible(el)) panelsBefore.add(el); });
    ...
    for (const f of fields) {
      ...
      if (written == null || String(written) === '') {
        skipped.push({ label: captionOf(f), type: f.type, lib: f.lib || '', source: entry.source, f, plannedValue: entry.value });
        continue;
      }
      // For a choice the plan is null ("pick one"); remember what was committed so a repair restores *that*.
      wrote.push({ f, key: fieldKey(f), caption: captionKey(f), value: entry.value == null ? String(written) : entry.value });
    ...
    for (let pass = 0; pass < 5; pass++) {
      ...
      for (const w of wrote) {
        ...
        if (again != null && String(again) !== '') { repaired++; didSomething = true; }
      }
      ...
      for (const f of fresh) {
        ...
        timings.push({ ms: Date.now() - tR, type: f.type, lib: (f.lib || 'native') + ':revealed', label: captionOf(f, 28) });
        if (written == null || String(written) === '') continue;
        wroteNow++; didSomething = true; settled = false;
        if (fromModel) aiUsed++;
        if (earlier) { earlier.f = f; repaired++; continue; }     // a repair, already listed
        revealed++;
    ...
    try {
      chrome.storage.local.set({ lastFill: {
        ...
        modelTimedOut, modelWarming, modelVia, modelAsked, leftOpen, notes,
    ...
    ping({ stage: 'done', text: `Filled ${filled.length} field${filled.length === 1 ? '' : 's'}`,
           done: filled.length, total: fields.length, aiUsed, skipped: skipped.length, ms: phase.total });
    return { count: filled.length, persona: stripRng(persona), aiUsed, widgets: widgetCount,
             revealed, repaired, leftOpen, notes, modelTimedOut, modelWarming, modelVia, modelDebug,
  ...
  // One field, on its own terms: no ordering, no repair loop, and worth a model round trip.
  async function fillOne(settings) {
    const usable = (n) => n && n.nodeType === 1 && n !== document.body && n !== document.documentElement && document.contains(n);
    const el = (usable(lastContextTarget) && lastContextTarget) || (usable(document.activeElement) && document.activeElement);
    if (!el) {
      ...
      return { ok: false, error: unseen ? 'nothing was under that right-click yet — try it again' : 'that right-click was not on a field' };
    }
    ...
    if (value == null && settings.useAI !== false) {
      ...
      if (v != null && String(v).trim() !== '') { value = v; source = 'ai'; }
    }
    ...
    const entry = { label: captionOf(f), value: String(written || '').slice(0, 60),
    ...
    return { ok, count: ok ? 1 : 0, persona: stripRng(persona), aiUsed,
  ...
  function clearAll() {
    ...
    document.querySelectorAll(`[${MARK}]`).forEach(el => {
      ...
      if (type === 'checkbox' || type === 'radio') {
        if (el.checked) { el.checked = false; fire(el, ['input', 'change']); n++; }
      } else if (el.isContentEditable) {
        el.textContent = ''; fire(el, ['input', 'change']); n++;
      } else if ('value' in el) {
        H.setNativeValue(el, '');
        fire(el, ['input', 'change']); n++;
      } else {
        const clear = el.querySelector('[class*="clear"], [data-pc-section="clearicon"]');
        if (clear) { H.press(clear); n++; }
      }
    });
  ...
  addEventListener('contextmenu', (e) => { sawContextMenu = true; lastContextTarget = e.target; }, true);

  if (typeof chrome === 'undefined' || !chrome.runtime || !chrome.runtime.onMessage) return;

  function exclusive(job, respond) {
    if (busy) { respond({ ok: false, error: 'a fill is already running' }); return false; }
    busy = true;
    job().then(respond).catch(e => respond({ ok: false, error: String(e) })).finally(() => { busy = false; });
    return true;
  }

  chrome.runtime.onMessage.addListener((msg, sender, respond) => {
    if (msg.kind === 'fill') return exclusive(async () => ({ ok: true, ...(await run(msg.settings || {})) }), respond);
    if (msg.kind === 'fill-one') return exclusive(() => fillOne(msg.settings || {}), respond);
    ...
    if (msg.kind === 'scan') {
      ...
      respond({
        ...
        widgets: fields.filter(f => f.kind === 'widget').map(f => ({ lib: f.lib, kind: f.type, label: f.label.slice(0, 60) })),
        fields: fields.map(f => ({
