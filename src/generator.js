...
(function () {
  ...
  function someSentences(r, langKey, n) {
    ...
    for (let i = 0; i < n && used.size < pool.length; i++) {
      let idx;
      do { idx = Math.floor(r() * pool.length); } while (used.has(idx));
      used.add(idx);
      out.push(pool[idx]);
    }
    return out;
  ...
  const RULES = [
    ...
    [/countrylist|countrycode|phonecountry/i, p => [p.country, p.countryEn, p.countryCode]],
    ...
    [/\b(country|land|staat)\b/i, p => [p.country, p.countryEn, p.countryCode]],
  ...
  function matchRuleDetail(text, persona) {
    ...
    for (const [re, fn, tag] of RULES) {
      ...
      try { return { value: fn(persona), pattern: String(re), weak: tag === WEAK }; }
      catch (_) { return null; }
    }
    return null;
  }
  ...
  function constrain(value, limits) {
    ...
    if (limits.pattern) {
      let re = null;
      try { re = new RegExp('^(?:' + limits.pattern + ')$'); } catch (_) { re = null; }
      if (re && !re.test(out)) {
  ...
  function byType(field, persona) {
    ...
    switch (field.type) {
      ...
      case 'date': return persona.futureDate;
      case 'datetime-local': return persona.futureDate + 'T10:30';
      case 'month': return persona.futureDate.slice(0, 7);
      case 'week': return persona.futureDate.slice(0, 4) + '-W12';
      case 'time': return `${pad(9 + Math.floor(r() * 9))}:${pad(Math.floor(r() * 12) * 5)}`;
      case 'color': return '#' + Math.floor(r() * 0xffffff).toString(16).padStart(6, '0');
      case 'textarea': return persona.sentence;
      default: return null;
    }
  }
