...
function parseValues(text, asked) {
  ...
  try { data = JSON.parse(text); }
    ...
    if (m) { try { data = JSON.parse(m[0]); } catch (_) { /* unparseable */ } }
  }
  if (!data) return out;
  ...
  for (const item of items) {
    ...
    if (id !== null && known.has(id)) { out[id] = clean(item.value); continue; }
...
async function nanoSessionGet({ allowDownload = false } = {}) {
  ...
  try { return await nanoPending; }
  finally { nanoPending = null; nanoBuilding = false; }
}
...
async function statelessSession(session) {
  ...
  try { return { s: await session.clone(), temporary: true }; }
  catch (_) { return { s: session, temporary: false }; }
}
...
async function generateOnDevice(persona, pageTitle, fields, context, examples, opts) {
  ...
  if (!session) {
    ...
    if (session && tabId != null) {
      chrome.tabs.sendMessage(tabId, { kind: 'model-stage', stage: 'asking',
...
async function askBatch(session, persona, pageTitle, group, context, examples) {
  ...
  let text = '';
  try {
    try { text = await turn.prompt(prompt, { responseConstraint: RESPONSE_SCHEMA }); }
    catch (_) { text = await turn.prompt(prompt); }
  } catch (err) {
    lastExchange.batches.push({ prompt, error: String(err && err.message || err), ms: Date.now() - t0 });
    return {};
  } finally {
    if (temporary) { try { turn.destroy(); } catch (_) { } }
  }
...
const PROVIDERS = {
  anthropic: {
    ...
    request: (cfg, model, prompt) => ({
      ...
      body: { model, max_tokens: 4000, system: SYSTEM_PROMPT, messages: [{ role: 'user', content: prompt }] }
    }),
...
async function generateRemote(cfg, persona, pageTitle, fields, context, examples) {
  ...
  for (const group of chunk(fields, BATCH * 2)) {
    ...
    try {
      ...
      if (!r.ok) throw new Error(`${cfg.provider} answered HTTP ${r.status}: ${JSON.stringify(j).slice(0, 300)}`);
      const text = provider.text(j);
      const parsed = parseValues(text, group);
      lastExchange.batches.push({ prompt, reply: String(text).slice(0, 4000), answered: Object.keys(parsed).length, asked: group.length, ms: Date.now() - t0 });
...
async function generate(payload, tabId) {
  ...
  if (backend !== 'remote-only') {
    const local = await generateOnDevice(persona, pageTitle, fields, context, examples, { sessionWaitMs, tabId, fieldCount: fields.length });
    if (local && Object.keys(local).length) return { ok: true, values: local, via: 'on-device', debug: lastExchange };
    if (backend === 'ondevice-only') return { ok: true, values: {}, via: 'none', warming: !!lastExchange.warming, debug: lastExchange };
  }
  if (cfg.apiKey && cfg.provider) {
    const remote = await generateRemote(cfg, persona, pageTitle, fields, context, examples);
    return { ok: true, values: remote, via: cfg.provider, debug: lastExchange };
  }
...
async function nanoCheck() {
  ...
  try {
    ...
    if (!session) { out.ok = false; out.note = 'no session could be built'; return out; }
...
async function showBooting(tabId) {
  ...
  try { await chrome.scripting.executeScript({ target: { tabId }, func: bootIndicator }); } catch (_) { }
}
...
function startSpin(tabId) {
  ...
  let frames;
  try { frames = spinnerFrames(); } catch (_) { return; }
  spinTab = tabId;
  spinFrame = 0;
  spinTimer = setInterval(() => {
    try {
      chrome.action.setIcon({ tabId, imageData: { 32: frames[spinFrame % frames.length] } }, () => void chrome.runtime.lastError);
    } catch (_) { stopSpin(tabId); }
    spinFrame++;
  }, SPIN_MS);
...
async function send(kind, settings, tabId) {
  ...
  try {
    try {
      return await chrome.tabs.sendMessage(id, { kind, settings });
    } catch (_) {
...
const MENUS = [
  { id: 'ff-fill-page', title: 'Fill this page with test data', contexts: ['page', 'editable', 'selection', 'link', 'image'] },
...
function installMenus() {
  chrome.contextMenus.removeAll(() => { for (const m of MENUS) chrome.contextMenus.create(m); });
}
...
chrome.runtime.onMessage.addListener((msg, sender, respond) => {
  ...
  if (msg.kind === 'nano-download') {
    nanoSessionGet({ allowDownload: true }).then(s => respond({ ok: !!s })).catch(e => respond({ ok: false, error: String(e) }));
    return true;
  }
  if (msg.kind === 'inject') {
  ...
  if (msg.kind === 'last-exchange') { respond({ ok: true, exchange: lastExchange }); return false; }
  if (msg.kind === 'nano-check') { nanoCheck().then(respond); return true; }
  if (msg.kind === 'nano-status') { nanoStatus().then(s => respond({ ok: true, status: s })); return true; }
  return false;
});
