...
if (!worker) { console.log('BUG   the service worker never registered'); process.exit(1); }
...
const result = await worker.evaluate(async ({ files, at }) => {
  ...
  const seen = await new Promise(r => setTimeout(async () => r(
    (await chrome.scripting.executeScript({ target: { tabId: tab.id }, func: () =>
      !!document.querySelector('[data-formforge-boot]') }))[0].result), 120));
...
const icon = await worker.evaluate(async () => {
  ...
  const err = await new Promise(r => { stopSpin(tab.id); setTimeout(() => r(null), 100); });
